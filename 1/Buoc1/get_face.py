...
save_dir = r"C:\Users\LENOVO\Downloads\Nam3.2\XU_LY_ANH_SO\Do-an\NhanDangKhuonMatOnnx\image\MinhHieu"
    ...
    while True:
        ...
        if key == ord('s') or key == ord('S'):
            if faces[1] is not None:
                ...
                file_name = os.path.join(save_dir, f"MinhHieu_{dem:04d}.bmp")
                cv.imwrite(file_name, face_align)
                print(f"Lưu ảnh: {file_name}")
                dem += 1
